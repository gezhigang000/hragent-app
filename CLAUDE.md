...
│       │                         # prompts (外部 .md 文件 + LazyLock 热重载, BASE + DAILY + STEP0~5),
│       │                         # prompt_guard (泄露检测), orchestrator (6步分析编排器)
│       ├── search/               # tavily, searxng, bing (三级降级搜索)
│       │                         # Bing CN (免费,优先) → SearXNG (免费,二级) → Tavily (付费,三级)
...
29. **Python 运行时打包（开箱即用）** — 使用 `python-build-standalone`（Astral 维护）将 Python 3.12 + pip 依赖打包进安装包（.app 244MB / .dmg 80MB）。`runner.rs` 的 `resolve_python_path()` 优先查找 `{resource_dir}/python-runtime/bin/python3`（macOS）或 `python-runtime/python.exe`（Windows），fallback 到系统 `python3`（开发模式）。打包模式下设置 `PYTHONHOME`、清除 `PYTHONPATH`。`scripts/setup-python.sh` / `setup-python.ps1` 负责下载 + 安装 + 瘦身（401→238MB，删除 tests/__pycache__/tkinter/pip 等）。pip 安装强制 `--only-binary :all:` 避免编译 C 扩展。`requirements.txt` 中已移除 `xhtml2pdf`（拉入 pycairo C 扩展链，实际未使用）。Windows 脚本含 3 次下载重试 + 文件大小验证 + tar 解压校验。

30. **系统提示词外部化（热重载）** — 系统提示词从 `prompts.rs` 硬编码迁移到 `src-tauri/prompts/*.md` 外部文件（base.md、daily.md、step0~step5.md），通过 `LazyLock<RwLock<PromptStore>>` 单例管理。Tauri 资源配置 `"prompts": "prompts"` 确保打包后可访问。开发时修改 `.md` 文件后调用 `reload_prompts()` 即可生效，无需重编译。

31. **系统提示词泄露检测（prompt_guard）** — `prompt_guard.rs` 检测 LLM 响应中是否包含系统提示词的关键片段。在 `finish_agent()` 保存消息前调用，泄露时记录 `log::warn!`。

32. **通用 OpenAI 兼容模型（custom-openai）** — 支持连接任意 OpenAI 兼容 API（Ollama、Groq、LM Studio、vLLM 等）。设置弹窗配置 Base URL + 模型名 + API Key（可选）+ 工具调用支持开关。`custom_openai.rs` 复用 `openai.rs` 的辅助函数。API Key 为空时不发送 Authorization header（本地模型兼容）。

33. **搜索三级降级** — Bing CN（`search/bing.rs`，HTML 抓取，中国大陆可用，免费）→ SearXNG（公共实例轮询）→ Tavily（付费 API Key）。`handle_web_search()` 依次尝试，全部失败返回 `[搜索不可用]` 提示 LLM 基于已有知识回答。
